...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.role !== tab) {
        setError(`This account is registered as "${data.role}", not "${tab}"`);
        setLoading(false);
        return;
      }
      login(data);
      navigate(data.role === "railway" ? "/railway" : "/engineer");
    } catch {
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center relative">
      ...
      <div className="w-full max-w-sm px-6 relative z-10">
        ...
        <p className="text-center text-[10px] text-muted-foreground/40 font-mono mt-6 tracking-wider">
          {tab === "railway" ? "user: railway / pass: railway123" : "Engineers: arjun, priya, vikram, kavitha, suresh, deepak / pass: eng123"}
        </p>
      </div>
    </div>
  );
