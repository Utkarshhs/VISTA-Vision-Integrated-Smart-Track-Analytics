...
import { LogOut, MapPin, AlertTriangle, CheckCircle2, Camera, Upload, Brain, ShieldAlert, Wrench, Loader2, Mic } from "lucide-react";
...
export default function EngineerDashboard() {
  ...
  const isChecklistComplete = checklist.possession && checklist.loto && checklist.ppe;
  ...
  const handleDictation = () => {
    ...
    if (!SpeechRecognition) {
      toast({ title: "Not Supported", description: "Voice dictation is not supported in this browser.", variant: "destructive" });
      return;
    }
    ...
    recognition.onstart = () => {
      setIsRecording(true);
      toast({ title: "Listening...", description: "Speak your repair feedback now." });
    };
    ...
    recognition.onerror = () => {
      toast({ title: "Microphone Error", description: "Could not record voice.", variant: "destructive" });
              ...
              <div className="p-5 border-b border-white/8">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <AlertTriangle className={cn("w-5 h-5", pendingTask.componentStatus?.includes('Severe Risk') ? "text-red-400" : "text-orange-400")} />
                    <span className={cn("text-sm font-display font-black tracking-wider uppercase", pendingTask.componentStatus?.includes('Severe Risk') ? "text-red-400" : "text-orange-400")}>
          ...
          <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              <div>
                ...
                <h2 className="text-3xl font-display font-black text-white">{activeTask.componentId || activeTask.component}</h2>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest mb-1">Component Type</p>
                  <p className="text-base font-medium text-white">{activeTask.componentType || "Unknown"}</p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest mb-1">CII Score</p>
                  <p className="text-xl font-display font-black" style={{ color: Number(activeTask.componentCII ?? 0) <= 30 ? "hsl(0 79% 50%)" : "hsl(25 95% 53%)" }}>
                    {activeTask.componentCII || "N/A"}
                  </p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest mb-1">Critical Level</p>
                  <div className="flex items-center gap-2 mt-0.5">
                    <AlertTriangle className={cn("w-4 h-4", activeTask.componentStatus?.includes('Severe Risk') ? "text-red-400" : "text-orange-400")} />
                    <p className={cn("text-sm font-bold uppercase", activeTask.componentStatus?.includes('Severe Risk') ? "text-red-400" : "text-orange-400")}>
                      {activeTask.componentStatus || "PRIORITY"}
                    </p>
                  </div>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  ...
                  <div className="flex items-center gap-1.5 text-sm text-white mt-0.5">
                    ...
                    <span className="truncate">See dispatch details</span>
                  </div>
                </div>
              </div>

              <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest mb-2">Assignment Info</p>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-white/60">Dispatched By:</span>
                    <span className="text-white">{activeTask.assignedBy || "System"}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-white/60">Deadline:</span>
                    <span className="text-white">{activeTask.deadline || "ASAP"}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-white/60">Assigned At:</span>
                    <span className="text-white">{new Date(activeTask.timestamp).toLocaleString()}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-4 relative overflow-hidden">
                ...
                <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest mb-3">Maintenance History</p>
                <div className="space-y-4">
                  <div className="flex gap-3">
                    <div className="w-2 h-2 rounded-full bg-primary/80 mt-1.5 flex-shrink-0"></div>
                    <div>
                      <p className="text-sm text-white">Minor realignment & fastener check</p>
                      <p className="text-xs text-muted-foreground">8 months ago • <span className="text-white/60">by Arjun</span></p>
                    </div>
                  </div>
                  <div className="flex gap-3">
                    <div className="w-2 h-2 rounded-full bg-white/20 mt-1.5 flex-shrink-0"></div>
                    <div>
                      <p className="text-sm text-white/70">Ultrasonic flaw detection passed</p>
                      <p className="text-xs text-muted-foreground">1.5 years ago • <span className="text-white/40">Automated Scan</span></p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {/* Right Column: Photo Upload & Feedback */}
            <div className="space-y-5">
              ...
              <div className="bg-white/3 border border-white/8 rounded-xl p-4">
                <p className="text-xs font-display font-semibold text-muted-foreground uppercase tracking-widest mb-3 flex items-center gap-2">
                  <ShieldAlert className="w-3.5 h-3.5" /> Mandatory Safety Checklist
                </p>
                <div className="space-y-2">
                  {[
                    { id: 'possession', label: 'Track possession block confirmed with Control' },
                    { id: 'loto', label: 'Lockout/Tagout (LOTO) protocols active' },
                    { id: 'ppe', label: 'High-visibility PPE & safety gear equipped' }
                  ].map(item => (
                    <label key={item.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 cursor-pointer transition-colors border border-transparent hover:border-white/5">
                      <input 
                        type="checkbox" 
                      ...
                      />
                      <span className={cn("text-sm transition-colors", checklist[item.id as keyof typeof checklist] ? "text-white" : "text-white/60")}>
                        {item.label}
                      </span>
                    </label>
                  ))}
              ...
              <div className={cn("bg-white/3 border rounded-xl p-4 transition-all duration-300", 
                isChecklistComplete ? "border-primary/30 shadow-[0_0_15px_hsl(0,82%,58%,0.1)]" : "border-white/8 opacity-50 pointer-events-none"
              )}>
                <div className="flex items-center justify-between mb-3">
                  <p className="text-xs font-display font-semibold text-muted-foreground uppercase tracking-widest flex items-center gap-2">
                    <Camera className="w-3.5 h-3.5" /> Upload Repair Photo
                  </p>
                  {!isChecklistComplete && <span className="text-[10px] text-red-400/80 font-mono">COMPLETE CHECKLIST FIRST</span>}
                </div>
                {photo ? (
                  <div className="relative">
                    <img src={photo} alt="Repair" className="w-full h-48 object-cover rounded-lg border border-white/10" />
              ...
              <div className={cn("bg-white/3 border border-white/8 rounded-xl p-4 transition-opacity relative", !isChecklistComplete && "opacity-50 pointer-events-none")}>
                <div className="flex items-center justify-between mb-3">
                  <p className="text-xs font-display font-semibold text-muted-foreground uppercase tracking-widest">Engineer Feedback</p>
                  <button onClick={handleDictation} title="Dictate Feedback"
                    className={cn("p-1.5 rounded-full transition-all flex items-center gap-1.5 px-3", 
                      isRecording ? "bg-red-500/20 text-red-400 animate-pulse border border-red-500/30" : "bg-white/5 text-muted-foreground hover:bg-white/10 hover:text-white border border-white/10"
                    )}>
                    <Mic className="w-3.5 h-3.5" />
                    <span className="text-[10px] font-mono uppercase tracking-widest">{isRecording ? "Listening..." : "Dictate"}</span>
                  </button>
                </div>
                <textarea value={feedback} onChange={e => setFeedback(e.target.value)}
                  placeholder="Describe what was wrong and how it was fixed... (or use Dictate button)"
              ...
              <Button onClick={handleSubmit} disabled={submitting || !feedback.trim() || !isChecklistComplete}
                className="w-full font-display uppercase tracking-widest h-12 transition-all"
                style={{ background: isChecklistComplete ? "hsl(0 82% 58%)" : "hsl(0 0% 20%)" }}>
